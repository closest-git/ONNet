...
def Z_max_pool2d(input_r,input_i,kernel_size, stride=None, padding=0,
...
def Z_dropout(input_r,input_i, p=0.5, training=True, inplace=False):
    return dropout(input_r, p, training, inplace), \
           dropout(input_i, p, training, inplace)


def Z_dropout2d(input_r,input_i, p=0.5, training=True, inplace=False):
...
def Z_fft(input, direction='C2C', inverse=False):
    """
...
def Z_Hadamard(A, B, inplace=False):
    """
